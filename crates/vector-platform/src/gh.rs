...
use crate::process::{build_env, resolve_program, ProgramRef, ProcessSpec};
...
pub fn spec_for(
    ...
    let program = resolve_program(program).map_err(|error| GhRefusal::Process(error.to_string()))?;
    let env = build_env(parent_env, &BTreeMap::new())
...
mod tests {
    ...
    fn the_lane_runs_a_real_process_and_parses_its_output() {
        ...
        assert_eq!(opened.command.first().map(String::as_str), Some(
