...
class Fetcher:
    def __init__(self, into: Path):
        self.into = into
        # One record per path: a work reached twice -- `book-of-wonders.txt` is both in the
        # Paragraph Comprehension manifest and the General Science source -- is one file, and a
        # manifest with the same path twice is a manifest nobody can join against.
    ...
    def _write(self, path: Path, body: bytes, url: str, note: str) -> None:
        ...
        self.records[str(path.relative_to(ROOT))] = {
...
def main(argv: list[str]) -> int:
    ...
    manifest = {
        ...
        "records": sorted(fetcher.records.values(), key=lambda record: record["path"]),
